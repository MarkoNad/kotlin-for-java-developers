...
class GameBoardImpl<T>(width: Int) : SquareBoardImpl(width), GameBoard<T> {
    ...
    override fun filter(predicate: (T?) -> Boolean): Collection<Cell> = cells.filter { cell -> predicate(cellToData[cell]) }.toList()

    override fun find(predicate: (T?) -> Boolean): Cell? = cells.find { cell -> predicate(cellToData[cell]) }
