...
fun GameBoard<Int?>.moveValuesInRowOrColumn(rowOrColumn: List<Cell>): Boolean {
    val movedCells = rowOrColumn
            .map { cell -> this[cell] }
            .moveAndMergeEqual { value -> value * 2 }

    val areMoved = movedCells.isNotEmpty() && (movedCells.size != rowOrColumn.size)

    if (areMoved) {
        rowOrColumn.forEach { set(it, null) }
        movedCells.forEachIndexed { index, value ->
